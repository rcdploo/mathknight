...
function digitMin(advanced: boolean) { return advanced ? 2 : 1; }
...
function stage1(variable: string, solution: number, advanced: boolean) {
  const min = digitMin(advanced);
  for (let guard = 0; guard < 500; guard += 1) {
    const coefficient = randomInt(min, 9); const constant = randomInt(min, 9); const sign = choice<1 | -1>([1, -1]);
    const right = coefficient * solution + sign * constant;
    if (right >= min && right <= 9) return `${coefficient}${variable} ${sign === 1 ? "+" : "−"} ${constant} = ${right}`;
  }
  throw new Error("Could not generate Stage 1 algebra equation.");
}

function stage2(variable: string, solution: number, advanced: boolean) {
  const min = digitMin(advanced);
  for (let guard = 0; guard < 1500; guard += 1) {
    ...
    const terms = kinds.map((isVariable) => ({ coefficient: randomInt(min, 9), variable: isVariable, sign: choice<1 | -1>([1, -1]) }));
    ...
    if (right >= min && right <= 9) return `${formatTerms(terms, variable)} = ${right}`;
  }
  throw new Error("Could not generate Stage 2 algebra equation.");
}

function stage3(variable: string, solution: number, advanced: boolean) {
  const min = digitMin(advanced);
  for (let guard = 0; guard < 1200; guard += 1) {
    const a = randomInt(min, 9); const c = randomInt(min, 9); const b = randomInt(min, 9);
    const d = (a - c) * solution + b;
    if (d < min || d > 9 || a === c) continue;
    if (Math.random() < 0.5 && b >= min * 2) {
      const first = randomInt(min, b - min); const second = b - first;
...
function stage4(variable: string, solution: number, advanced: boolean) {
  const min = digitMin(advanced);
  for (let guard = 0; guard < 2500; guard += 1) {
    const a = randomInt(min, 9); const c = randomInt(min, 9); const denominator = randomInt(2, 9); const e = randomInt(min, 9);
    if (a === c * denominator) continue;
    const b = denominator * (c * solution + e) - a * solution;
    if (b < min || b > 9) continue;
...
function stage5(variable: string, solution: number, advanced: boolean) {
  const min = digitMin(advanced);
  for (let guard = 0; guard < 4000; guard += 1) {
    const a = randomInt(min, 9); const b = randomInt(min, 9); const c = randomInt(min, 9);
    ...
    if (e < min || e > 9 || leftDenominator === rightDenominator) continue;
...
function makeEquation(stage: Stage, variable: string, solution: number, advanced: boolean) {
  if (stage === "1") return stage1(variable, solution, advanced);
  if (stage === "2") return stage2(variable, solution, advanced);
  if (stage === "3a") return stage3(variable, solution, advanced);
  if (stage === "3b") return stage4(variable, solution, advanced);
  return stage5(variable, solution, advanced);
}
...
export function generateAlgebraPuzzle(level: LevelConfig): PuzzleCard[] {
  ...
  for (let index = 0; index < level.pairs; index += 1) {
    let guard = 0;
    while (guard < 500) {
      const variable = levelVariables[index % levelVariables.length]; const solution = randomInt(digitMin(advanced), 9); const answer = `${variable} = ${solution}`;
      if (!usedAnswers.has(answer)) {
        const equation = makeEquation(level.stage, variable, solution, advanced);
